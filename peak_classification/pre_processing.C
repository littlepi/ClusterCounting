...
void plot_wf(vector<double> wf, vector<double> time, TString label = "") {
    ...
    c->Print(Form("wf_%d%s.png", id, label.Data()));
    delete c;
...
void plot_sample(vector<double> sample, TString label = "") {
    ...
    c->Print(Form("sample_%d%s.png", id, label.Data()));
    delete c;

    id++;
}

void pre_processing(TString filename = "signal.root", TString outfile = "dataset.txt", int start = 0, int n = -1, int type = 0, bool isData = false) { // tag 0: supervised, tag 1: only signal, tag 2: only background
